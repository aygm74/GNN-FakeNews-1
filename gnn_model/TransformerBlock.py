...
class InputEmbbeding(nn.Module):
    def __init__(self, d_model:int, time_size:int) -> None:
        super().__init__()
        self.d_model = d_model
        self.time_size = time_size
        self.embbeding = nn.Embedding(time_size, d_model)
...
class PossitionalEncodding(nn.Module):
    def __init__(self, d_model:int, seq_len:int, dropout:float) -> None:
        ...
        #create a matrix of position of each time shape (seq_len,d_model)
        ...
        #(batch, seq_len, d_mdoel)
        pe.unsqueeze(0) #(1, seq_len, d_mdoel)

        # save matrix_psition no as parameter when save state of model     
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k ,v):
        ...
        x , attention_score = MultiHeadAttention.attention(query, key, value , self.mask, self.dropout)
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x,sublayer):
            return x + self.dropout(sublayer(self.norm(x))) # in most of  implementation first norm x and then forward to sublayer    
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, self_attention_block: MultiHeadAttention,feed_forward_block: FeedForwardBlock, dropout:float ) -> None:
            ...
            self.residualconnection = nn.ModuleList([ResidualConnection for _ in range(2)])

    # src_mask is for doesn't attention time to padding       
